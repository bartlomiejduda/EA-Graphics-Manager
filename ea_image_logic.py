    ...
    def convert_images(self):
        ...
        for i in range(self.num_of_entries):
            ...
            if entry_type not in conv_images_supported_types:
                logger.console_logger("Warning! Entry type " + str(entry_type) + " is not supported" +
                                      " for image conversion! Skipping!")
                continue
                
            else:
                logger.console_logger("Converting image type " + str(entry_type) + ", number " + str(i+1) + "...")
            ...
            if entry_type == 2:
                ...
                for i in range(img_height):
                    ...
                    for j in range(img_width):
                        pixel = struct.pack(self.f_endianess + "B", ea_dir_entry.raw_data[read_count])
                        read_count += 1
                        temp_row += pixel
                    if skew_val == 1:
                        temp_row += b'\x00\x00'
                ...
                diff = (ea_dir_entry.h_size_of_the_block - ea_dir_entry.header_size) - img_size_calc
                for i in range(diff):
                    pixel = b'\x00'
                    img_data += pixel
                ...
                for i in range(pal_range):
                    pal_entry1 = struct.pack(self.f_endianess + "B", ea_dir_entry.bin_attachments_list[0].raw_data[read_count])
                    read_count += 1
                    pal_entry2 = struct.pack(self.f_endianess + "B", ea_dir_entry.bin_attachments_list[0].raw_data[read_count])
                    read_count += 1
                    pal_entry3 = struct.pack(self.f_endianess + "B", ea_dir_entry.bin_attachments_list[0].raw_data[read_count])
                    read_count += 1
                    pal_entry4 = struct.pack(self.f_endianess + "B", ea_dir_entry.bin_attachments_list[0].raw_data[read_count])
                    read_count += 1                    
                    img_pal_data += pal_entry3 + pal_entry2 + pal_entry1 + pal_entry4 # RGBA swap  
                ...
                bmp_object = BMP_IMG(img_width, img_height, img_bpp, img_data, img_pal_data)
